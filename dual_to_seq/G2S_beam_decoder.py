...
def run_beam_search(sess, model, vocab, batch, options):
    # Run encoder
    (encoder_states, encoder_features, node_idx, node_mask,
    ...
    context_encoder_t = np.zeros([model.encoder_dim]) # [encode_dim]
    context_src_t = np.zeros([model.src_hidden_dim]) # [src_dim]
    coverage_t = np.zeros((src_states.shape[1])) # [src_len]
    hyps = []
    ...
    while steps < options.max_answer_len and len(results) < options.beam_size:
        ...
        cur_coverage_t_1 = [] # [batch_size, src_len]
...
if __name__ == '__main__':
    ...
    devDataStream = G2S_data_stream.G2SDataStream(testset, word_vocab_enc, word_vocab_dec, char_vocab, edgelabel_vocab, options=FLAGS,
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope("Valid"):
            with tf.variable_scope("Model", reuse=False, initializer=initializer):
                valid_graph = ModelGraph(word_vocab_enc=word_vocab_enc, word_vocab_dec=word_vocab_dec, char_vocab=char_vocab,
                        Edgelabel_vocab=edgelabel_vocab, options=FLAGS, mode="decode")
