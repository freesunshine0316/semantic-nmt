...
def read_amr_file(inpath, options, word_vocab_enc, word_vocab_dec, char_vocab, edgelabel_vocab):
    ...
    with open(inpath, "rU") as f:
        for id, inst in enumerate(json.load(f)):
            ...
            amr_node = dep['tokens'] # dependency is always shorter (<=50) than threshold (80)
            ...
            amr_edge = zip(dep['parents'],
                    edgelabel_vocab.to_index_sequence_for_list(dep['labels']))
            ...
            for j,(i,lb) in enumerate(amr_edge):
                if i == -1: # root node
                    continue
...
if __name__ == "__main__":
    ...
    all_instances, _, _, _, _ = read_amr_file('../data/all.tok_le50.json', FLAGS,
            word_vocab_enc, word_vocab_dec, None, edgelabel_vocab)
    print('Build DataStream ... ')
    trainDataStream = G2S_data_stream.G2SDataStream(trainset, word_vocab_enc, word_vocab_dec, None, edgelabel_vocab,
