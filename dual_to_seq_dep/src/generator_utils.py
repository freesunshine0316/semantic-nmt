...
class CovCopyAttenGen:
    def __init__(self, placeholders, options, vocab, is_training=True):
    ...
    def one_step_decoder(self, state_t_1, context_encoder_t_1, context_src_t_1, coverage_t_1, word_t,
            encoder_states, encoder_features, node_idxs, node_mask,
            src_states, src_features, src_mask,
            v_enc, v_src, w_c, vocab):
        '''
        ...
        x = linear([word_t, context_encoder_t_1, context_src_t_1], options.attention_vec_size, True)

        # Run the decoder RNN cell. cell_output = decoder state
        cell_output, state_t = self.cell(x, state_t_1)

        with variable_scope.variable_scope("encoder_attn"):
            context_encoder_t, _, _ = self.attention(cell_output, options.attention_vec_size, encoder_states,
                    encoder_features, node_mask, v_enc, w_c=w_c,
                    use_coverage=False, coverage=None) # don't use coverage on AMR side
            #context_encoder_t = tf.zeros_like(context_encoder_t_1)

        with variable_scope.variable_scope("src_attn"):
            context_src_t, attn_dist, coverage_t = self.attention(cell_output, options.attention_vec_size, src_states,
                    src_features, src_mask, v_src, w_c=w_c,
                    use_coverage=options.use_coverage, coverage=coverage_t_1)
        # Calculate p_gen, Equation (8)
        p_gen = None
        if options.pointer_gen:
            assert False, 'NO pgen for MT'
            with tf.variable_scope('calculate_pgen'):
                p_gen = linear([context_src_t, state_t.c, state_t.h, x], 1, True) # [batch_size, 1]
                p_gen = tf.sigmoid(p_gen)

        # Concatenate the cell_output (= decoder state) and the context vector, and pass them through a linear layer
        # This is V[s_t, h*_t] + b in the paper
        with variable_scope.variable_scope("AttnOutputProjection"):
            output_t = linear([cell_output] + [context_encoder_t] + [context_src_t], options.gen_hidden_size, True)
        ...
        return (state_t, context_encoder_t, context_src_t, coverage_t, attn_dist, p_gen, vocab_score_t)

    def train_mode(self, vocab,
            encoder_dim, encoder_states, encoder_features, node_idxs, node_mask,
            src_hidden_dim, src_hiddens, src_features, src_mask,
        ...
        context_encoder_t_1 = tf.zeros([batch_size, encoder_dim])
        context_src_t_1 = tf.zeros([batch_size, src_hidden_dim])
        coverage_t_1 = None
        ...
        with variable_scope.variable_scope("attention_decoder"):
            ...
            v_enc = variable_scope.get_variable("v_enc", [options.attention_vec_size])
            v_enc = tf.expand_dims(tf.expand_dims(v_enc, axis=0), axis=0)
            v_src = variable_scope.get_variable("v_src", [options.attention_vec_size])
            v_src = tf.expand_dims(tf.expand_dims(v_src, axis=0), axis=0)
            w_c = None
            ...
            for i in range(options.max_answer_len):
                ...
                (state_t, context_encoder_t, context_src_t, coverage_t, attn_dist_t, p_gen_t, output_t) = self.one_step_decoder(
                                state_t_1, context_encoder_t_1, context_src_t_1, coverage_t_1, word_t,
                                encoder_states, encoder_features, node_idxs, node_mask,
                                src_hiddens, src_features, src_mask,
                                v_enc, v_src, w_c, vocab)
                ...
                state_t_1 = state_t
                context_encoder_t_1 = context_encoder_t
                context_src_t_1 = context_src_t
    ...
    def decode_mode(self, word_vocab, beam_size, state_t_1, context_encoder_t_1, context_src_t_1, coverage_t_1, word_t,
                    encoder_states, encoder_features, node_idxs, node_mask, src_states, src_features, src_mask):
        options = self.options
        ...
        with variable_scope.variable_scope("attention_decoder"):
            ...
            (state_t, context_encoder_t, context_src_t, coverage_t, attn_dist_t, p_gen_t, output_t) = self.one_step_decoder(
                    state_t_1, context_encoder_t_1, context_src_t_1, coverage_t_1, word_t_representation,
                    encoder_states, encoder_features, node_idxs, node_mask,
                    src_states, src_features, src_mask, v_enc, v_src, w_c, word_vocab)
        ...
        return (state_t, context_encoder_t, context_src_t, coverage_t, attn_dist_t, p_gen_t, output_t, topk_log_probs, topk_ids,
...
def CE_loss_v2(word_probs, answers, loss_weights):
    epsilon = 1.0e-6
    word_probs = _clip_and_normalize(word_probs, epsilon)
    xent = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=answers, logits=tf.log(word_probs))
