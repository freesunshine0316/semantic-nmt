...
class SeqEncoder(object):
    ...
    def encode(self, is_training=True):
        ...
        if self.word_vocab is not None:
            ...
            assert False, 'must use word embeddings'
        ...
        if False and options.with_POS and self.POS_vocab is not None:
        ...
        if False and options.with_NER and self.NER_vocab is not None:
        ...
        #w_memory = tf.get_variable('w_memory', [passage_dim, options.context_lstm_dim], dtype=tf.float32)
        #b_memory = tf.get_variable('b_memory', [options.context_lstm_dim], dtype=tf.float32)
        #all_passage_representation = tf.reshape(all_passage_representation, [-1, passage_dim])
        #all_passage_representation = tf.matmul(all_passage_representation, w_memory) + b_memory
        #all_passage_representation = tf.reshape(all_passage_representation, [batch_size, passage_len, options.context_lstm_dim])
