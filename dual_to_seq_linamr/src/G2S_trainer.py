...
def main(_):
    ...
    if FLAGS.infile_format == 'fof':
        trainset, trn_linamr, trn_src, trn_sent = G2S_data_stream.read_amr_from_fof(FLAGS.train_path)
    else:
        trainset, trn_linamr, trn_src, trn_sent = G2S_data_stream.read_amr_file(FLAGS.train_path)
    print('Number of training samples: {}'.format(len(trainset)))

    print('Loading test set.')
    if FLAGS.infile_format == 'fof':
        testset, tst_linamr, tst_src, tst_sent = G2S_data_stream.read_amr_from_fof(FLAGS.test_path)
    else:
        testset, tst_linamr, tst_src, tst_sent = G2S_data_stream.read_amr_file(FLAGS.test_path)
    print('Number of test samples: {}'.format(len(testset)))

    max_linamr = max(trn_linamr, tst_linamr)
    max_src = max(trn_src, tst_src)
    max_sent = max(trn_sent, tst_sent)
    print('Max linamr length: {}, truncated to {}'.format(max_linamr, FLAGS.max_linamr_len))
    print('Max src length: {}, truncated to {}'.format(max_src, FLAGS.max_src_len))
    print('Max sent length: {}, truncated to {}'.format(max_sent, FLAGS.max_answer_len))
    ...
    trainDataStream = G2S_data_stream.G2SDataStream(trainset, word_vocab_enc, word_vocab_dec,
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope("Train"):
            with tf.variable_scope("Model", reuse=None, initializer=initializer):
                train_graph = ModelGraph(word_vocab_enc=word_vocab_enc, word_vocab_dec=word_vocab_dec, options=FLAGS, mode=FLAGS.mode)

        assert FLAGS.mode in ('ce_train', )
        valid_mode = 'evaluate' if FLAGS.mode == 'ce_train' else None

        with tf.name_scope("Valid"):
            with tf.variable_scope("Model", reuse=True, initializer=initializer):
                valid_graph = ModelGraph(word_vocab_enc=word_vocab_enc, word_vocab_dec=word_vocab_dec, options=FLAGS, mode=valid_mode)
