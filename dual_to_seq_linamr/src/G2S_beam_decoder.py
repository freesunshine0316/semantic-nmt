...
def search(sess, model, vocab, batch, options, decode_mode='greedy'):
    ...
    (encoder_states, encoder_features, node_mask, initial_state) = model.run_encoder(sess, batch, options)
    ...
    for i in xrange(options.max_answer_len):
        ...
        feed_dict[model.src_hiddens] = src_states
        feed_dict[model.src_features] = src_features
        feed_dict[model.src_mask] = src_mask
...
class Hypothesis(object):
    def __init__(self, tokens, log_ps, attn, state, context_encoder_vector, context_src_vector, coverage_vector=None):
        self.tokens = tokens # store all tokens
        self.log_probs = log_ps # store log_probs for each time-step
        self.attn_ids = attn
        self.state = state
        self.context_encoder_vector = context_encoder_vector
        self.context_src_vector = context_src_vector
        self.coverage_vector = coverage_vector

    def extend(self, token, log_prob, attn_i, state, context_encoder_vector, context_src_vector, coverage_vector=None):
        return Hypothesis(self.tokens + [token], self.log_probs + [log_prob], self.attn_ids + [attn_i], state,
                          context_encoder_vector, context_src_vector, coverage_vector=coverage_vector)
...
def run_beam_search(sess, model, vocab, batch, options):
    # Run encoder
    (encoder_states, encoder_features, encoder_mask,
            src_states, src_features, src_mask, initial_state) = model.run_encoder(sess, batch, options)
    # encoder_states: [1, passage_len, encode_dim]
    # encoder_features: [1, passage_len, attention_vec_size]
    # encoder_mask: [1, passage_len]
    # initial_state: a tupel of [1, gen_dim]

    sent_stop_id = vocab.getIndex('</s>')

    # Initialize this first hypothesis
    context_encoder_t = np.zeros([model.linamr_hidden_dim]) # [encode_dim]
    context_src_t = np.zeros([model.src_hidden_dim]) # [src_dim]
    coverage_t = np.zeros((src_states.shape[1])) if options.use_coverage else None # [src_len]
    hyps = []
    hyps.append(Hypothesis([batch.sent_inp[0][0]], [0.0], [-1], initial_state, context_encoder_t, context_src_t, coverage_vector=coverage_t))
    ...
    while steps < max_steps and len(results) < options.beam_size:
        ...
        cur_encoder_mask = np.tile(encoder_mask, (cur_size, 1)) # [batch_size, passage_len]

        cur_src_states = np.tile(src_states, (cur_size, 1, 1))
        cur_src_features = np.tile(src_features, (cur_size, 1, 1)) # [batch_size,passage_len, options.attention_vec_size]
        cur_src_mask = np.tile(src_mask, (cur_size, 1)) # [batch_size, passage_len]

        cur_state_t_1 = [] # [2, gen_steps]
        cur_context_encoder_t_1 = [] # [batch_size, encoder_dim]
        cur_context_src_t_1 = [] # [batch_size, src_dim]
        cur_coverage_t_1 = [] if options.use_coverage else None # [batch_size, src_len]
        cur_word_t = [] # [batch_size]
        for h in hyps:
            cur_state_t_1.append(h.state)
            cur_context_encoder_t_1.append(h.context_encoder_vector)
            cur_context_src_t_1.append(h.context_src_vector)
            cur_word_t.append(h.latest_token())
            if options.use_coverage:
                cur_coverage_t_1.append(h.coverage_vector)
        cur_context_encoder_t_1 = np.stack(cur_context_encoder_t_1, axis=0)
        cur_context_src_t_1 = np.stack(cur_context_src_t_1, axis=0)
        if options.use_coverage:
        ...
        feed_dict[model.context_encoder_t_1] = cur_context_encoder_t_1
        feed_dict[model.context_src_t_1] = cur_context_src_t_1
        feed_dict[model.word_t] = cur_word_t

        feed_dict[model.linamr_hiddens] = cur_encoder_states
        feed_dict[model.linamr_features] = cur_encoder_features
        feed_dict[model.linamr_mask] = cur_encoder_mask

        feed_dict[model.src_hiddens] = cur_src_states
        feed_dict[model.src_features] = cur_src_features
        feed_dict[model.src_mask] = cur_src_mask

        if options.use_coverage:
            feed_dict[model.coverage_t_1] = cur_coverage_t_1

            (state_t, context_encoder_t, context_src_t, attn_dist_t, coverage_t, topk_log_probs, topk_ids) = sess.run([model.state_t,
                model.context_encoder_t, model.context_src_t, model.attn_dist_t,
                model.coverage_t, model.topk_log_probs, model.topk_ids], feed_dict)
        else:
            (state_t, context_encoder_t, context_src_t, attn_dist_t, topk_log_probs, topk_ids) = sess.run([model.state_t,
                model.context_encoder_t, model.context_src_t, model.attn_dist_t,
                model.topk_log_probs, model.topk_ids], feed_dict)
        ...
        for i in xrange(cur_size):
            ...
            cur_context_encoder = context_encoder_t[i]
            cur_context_src = context_src_t[i]
            cur_coverage = coverage_t[i] if options.use_coverage else None
            for j in xrange(options.beam_size):
                ...
                new_hyp = h.extend(cur_tok, cur_tok_log_prob, cur_attn_i, cur_state,
                        cur_context_encoder, cur_context_src, coverage_vector=cur_coverage)
...
if __name__ == '__main__':
    ...
    devDataStream = G2S_data_stream.G2SDataStream(testset, word_vocab_enc, word_vocab_dec,
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope("Valid"):
            with tf.variable_scope("Model", reuse=False, initializer=initializer):
                valid_graph = ModelGraph(word_vocab_enc=word_vocab_enc, word_vocab_dec=word_vocab_dec, options=FLAGS, mode="decode")
        ...
        for i in range(total_num):
            ...
            if mode in ['greedy', 'multinomial']:
                ...
                (sentences, prediction_lengths, generator_input_idx,
                 generator_output_idx) = search(sess, valid_graph, word_vocab_dec, cur_batch, FLAGS, decode_mode=mode)
                ...
                hyps = run_beam_search(sess, valid_graph, word_vocab_dec, cur_batch, FLAGS)
                outfile.write(' '.join(cur_batch.instances[0][-2]).encode('utf-8') + "\n")
                for j in xrange(1):
                    hyp = hyps[j]
                    cur_sent = hyp.idx_seq_to_string(None, word_vocab_dec, FLAGS)
