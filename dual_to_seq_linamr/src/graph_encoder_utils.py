...
class GraphEncoder(object):
    def __init__(self, word_vocab=None, edge_label_vocab=None, char_vocab=None, is_training=True, options=None, device_str='/gpu:0'):
        assert options != None
        print('Graph encoder use device ' + device_str)
        ...
        with tf.device(device_str):
            ...
            if options.compress_input: # compress input word vector into smaller vectors
                ...
                b_compress = tf.get_variable("b_compress_input", [options.compress_input__dim], dtype=tf.float32)
        ...
        with tf.variable_scope('graph_encoder'), tf.device(device_str):
            # =========== in neighbor
