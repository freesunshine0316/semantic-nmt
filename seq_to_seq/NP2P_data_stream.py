...
class QADataStream(object):
    def __init__(self, all_questions, enc_word_vocab=None, dec_word_vocab=None, char_vocab=None, POS_vocab=None, NER_vocab=None, options=None,
        ...
        for (sent1, sent2, sent3) in all_questions:# sent1 is the long passage or article
            sent1.convert2index(enc_word_vocab, char_vocab, POS_vocab, NER_vocab, max_char_per_word=options.max_char_per_word)
            if options.max_passage_len != -1 and sent1.get_length() > options.max_passage_len:
                sent1.chunk(options.max_passage_len)
            sent2.convert2index(dec_word_vocab, char_vocab, POS_vocab, NER_vocab, max_char_per_word=options.max_char_per_word)
            if sent3 is not None:
                sent3.convert2index(enc_word_vocab, char_vocab, POS_vocab, NER_vocab, max_char_per_word=options.max_char_per_word)
...
class QAQuestionBatch(object):
    def __init__(self, instances, options, word_vocab=None, char_vocab=None, POS_vocab=None, NER_vocab=None):
        ...
        self.id = [instances[i][0].ID_num for i in range(self.batch_size)]
        self.source = [instances[i][0].tokText for i in range(self.batch_size)]
        self.target_ref = [instances[i][1].tokText for i in range(self.batch_size)]
        ...
        for (sent1, sent2, sent3) in instances:
            ...
            self.sent2_length.append(min(sent2.get_length()+1, options.max_answer_len))
        ...
        if options.with_word:
            ...
            for (sent1, sent2, sent3) in instances:
                ...
                self.sent2_word.append(sent2.word_idx_seq+[end_id])
                self.sent2_input_word.append([start_id]+sent2.word_idx_seq)
        ...
        if options.with_char:
            ...
            self.sent2_char_lengths = []
            if self.has_sent3:
                self.sent3_char = [] # [batch_size, sent3_len]
                self.sent3_char_lengths = []
            for (sent1, sent2, sent3) in instances:
                self.sent1_char.append(sent1.char_idx_seq)
                self.sent2_char.append(sent2.char_idx_seq)
                self.sent1_char_lengths.append([len(x) for x in sent1.char_idx_seq])
                self.sent2_char_lengths.append([len(x) for x in sent2.char_idx_seq])
                if self.has_sent3:
                    self.sent3_char.append(sent3.char_idx_seq)
                    self.sent3_char_lengths.append([len(x) for x in sent3.char_idx_seq])
            ...
            self.sent2_char_lengths = padding_utils.pad_2d_vals_no_size(self.sent2_char_lengths)
            if self.has_sent3:
                self.sent3_char = padding_utils.pad_3d_vals_no_size(self.sent3_char)
                self.sent3_char_lengths = padding_utils.pad_2d_vals_no_size(self.sent3_char_lengths)
...
if __name__ == "__main__":
    all_instances, _ = read_all_GenerationDatasets('./data/all.tok_le50_bpe.json', True)
    print('DONE!')
    print('DONE!')
