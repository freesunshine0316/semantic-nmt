...
def main(_):
    ...
    trainDataStream = NP2P_data_stream.QADataStream(trainset, enc_word_vocab, dec_word_vocab, char_vocab, POS_vocab, NER_vocab,
            options=FLAGS, isShuffle=True, isLoop=True, isSort=True)

    devDataStream = NP2P_data_stream.QADataStream(testset, enc_word_vocab, dec_word_vocab, char_vocab, POS_vocab, NER_vocab,
    ...
    best_loss = FLAGS.best_loss if FLAGS.__dict__.has_key('best_loss') else 10000000.0
    if best_accu > 0.0:
        print('With initial dev accuracy {}'.format(best_accu))
    if best_bleu > 0.0:
        print('With initial dev BLEU score {}'.format(best_bleu))
    if best_loss < 10000000.0:
        print('With initial dev loss score {}'.format(best_loss))

    with tf.Graph().as_default():
        initializer = tf.random_uniform_initializer(-init_scale, init_scale)
        with tf.name_scope("Train"):
            with tf.variable_scope("Model", reuse=None, initializer=initializer):
                train_graph = ModelGraph(enc_word_vocab=enc_word_vocab, dec_word_vocab=dec_word_vocab, char_vocab=char_vocab, POS_vocab=POS_vocab,
